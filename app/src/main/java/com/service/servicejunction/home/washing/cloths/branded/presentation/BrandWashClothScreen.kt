...
//    val scrollState =  rememberScrollState()
    ...
    Column(
        ...
        SelectedSection(state, viewModel)
    ...
    Scaffold(
        floatingActionButton = {
            FloatingActionButton(
                ...
                Icon(Icons.Default.AddCircle, contentDescription = "Open Camera")
            }
        }
    ) { paddingValue ->
        if (showCamera) {
            CameraDialog(onDismiss = { showCamera = false }, Modifier.padding(paddingValue))
        }
    }
}

@Composable
fun CameraDialog(onDismiss: () -> Unit, padding: Modifier) {
